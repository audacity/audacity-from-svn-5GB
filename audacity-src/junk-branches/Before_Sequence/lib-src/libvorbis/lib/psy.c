 ...
 ********************************************************************
 ...
 last mod: $Id: psy.c,v 1.3 2002-04-21 23:45:31 habes Exp $
...
void _vp_compute_mask(vorbis_look_psy *p,
  ...
  if(p->vi->noisemaskp){
    ...
    bark_noise_hybridmp(n,(const long*)p->bark,logmdct,logmask,
			140.,-1);

    for(i=0;i<n;i++)work[i]=logmdct[i]-logmask[i];

    bark_noise_hybridmp(n,(const long *)p->bark,work,logmask,0.,
