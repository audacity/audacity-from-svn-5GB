...
//
// ---------- EXPERIMENTAL_NOTE_TRACK comments: ---------- 
//
// SERIALBUFFER CLASS
// 
...
// the library are:
//    Alg_track::serialize_track()
//    Alg_seq::serialize_seq()
//    Alg_track::serialize_parameter(Alg_parameter *parm)
//
// These internal serialize functions append data to ser_buf. 
//  
// UNSERIALIZATION:
//
...
//
// ---------- end of EXPERIMENTAL_NOTE_TRACK comments ---------- 
...
// (Alg_)Attribute is an atom in the symbol table
...
// an attribute/value pair. Since (Alg_)Attribute names imply type,
// we try to keep attributes and values packaged together as
// (Alg_)Parameter class
...
public:
    ...
    // Try to read from a memory buffer.
    ...
    // a difference because the pasted data may change the tempo,
...
// (Alg_)Time_sig represents a single time signature;
...
// (Alg_)Time_sigs is a dynamic array of time signatures
//
// Regarding Alg_time_sigs:
...
public:
    ...
    void beat_to_measure(double beat, long *measure, double *m_beat,
                         double *num, double *den);
    void set_events(Allegro_event_ptr *events, long len, long maxlen);
} *Seq_ptr;
...
public:
    ...
    void beat_to_measure(double beat, long *measure, double *m_beat,
                         double *num, double *den);
    // void set_events(Alg_event_ptr *events, long len, long maxlen);
