...
AudioEffect::AudioEffect(audioMasterCallback audioMaster, long numPrograms,
                         long numParams)
{
...
//-----------------------------------------------------------------------------
long AudioEffect::dispatcher(long opCode, long index, long value,
                             void *ptr, float opt)
{
   long v = 0;
   ...
   case effEditDraw:
      if (editor)
         editor->draw((ERect *) ptr);
      break;
   case effEditMouse:
      if (editor)
         v = editor->mouse(index, value);
      break;
   case effEditKey:
      if (editor)
         v = editor->key(value);
   ...
   case effGetChunk:
      ...
      break;
   case effSetChunk:
      v = setChunk(ptr, value, index ? true : false);
      break;
   }
   return v;
}
...
bool AudioEffect::isOutputConnected(long output)
{
   ...
   if (audioMaster)
      ret =
          audioMaster(&cEffect, audioMasterPinConnected, output, 1, 0, 0);
   return ret ? false : true;   // return value is 0 for true
}
