...
static off_t msadpcm_seek   (SF_PRIVATE *psf, off_t offset, int whence) ;
...
static int		
msadpcm_read_block (SF_PRIVATE *psf, MSADPCM_PRIVATE *pms)
...
static off_t    
msadpcm_seek   (SF_PRIVATE *psf, off_t offset, int whence)
{	MSADPCM_PRIVATE *pms ; 
...
{	MSADPCM_PRIVATE	*pms ;
	...
	blockalign      = srate2blocksize (psf->sf.samplerate) ;	
...
static int		
msadpcm_write_block (SF_PRIVATE *psf, MSADPCM_PRIVATE *pms)
...
static	unsigned int 
srate2blocksize (unsigned int srate)
{	if (srate < 12000)
		return 256 ;
	if (srate < 23000)
		return 512 ;
