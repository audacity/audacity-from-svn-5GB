...

{
   ...
   CMD_ADDMENUSEP(mExportString, "Export As", OnExportMix, fileMenu, enabledMenu);
   CMD_ADDMENU(mExportSelectionString, "Export Selection As", OnExportSelection, fileMenu, enabledMenu);

   CMD_ADDMENUSEP(mExportLossyString, "Export As (lossy)", OnExportLossyMix, fileMenu, enabledMenu);
   CMD_ADDMENU(mExportSelectionLossyString, "Export Selection As (lossy)", OnExportLossySelection, fileMenu, enabledMenu);

   CMD_ADDMENUSEP("Export &Labels...", "Export Labels", OnExportLabels, fileMenu, enabledMenu);

   CMD_ADDMENUSEP("Preferences...\tCtrl+P", "Preferences", OnPreferences, fileMenu, enabledMenu);

   CMD_ADDMENUSEP("E&xit", "Exit", OnExit, fileMenu, enabledMenu);

   // Edit menu
   CMD_ADDMENU("Undo\tCtrl+Z", "Undo", Undo, editMenu, enabledMenu);
   CMD_ADDMENU("Redo\tCtrl+R", "Redo", Redo, editMenu, enabledMenu);

   CMD_ADDMENUSEP("Cut\tCtrl+X", "Cut", Cut, editMenu, enabledMenu);
   ...
   CMD_ADDMENUSEP("Delete\tCtrl+K", "Delete", OnDelete, editMenu, enabledMenu);
   CMD_ADDMENU("Silence\tCtrl+L", "Silence", OnSilence, editMenu, enabledMenu);

   CMD_ADDMENUSEP("Insert Silence...", "Insert Silence", OnInsertSilence, editMenu, enabledMenu);
   ...
   CMD_ADDMENUSEP("Select All\tCtrl+A", "Select All", OnSelectAll, editMenu, enabledMenu);
   ...
   CMD_ADDMENUSEP("History", "Undo History", UndoHistory, viewMenu, enabledMenu);
   CMD_ADDMENU("Plot Spectrum\tCtrl+U", "Plot Spectrum", OnPlotSpectrum, viewMenu, enabledMenu);

#ifndef __WXMAC__
   CMD_ADDMENUSEP("Float Control Toolbar", "Float Control Toolbar", OnFloatControlToolBar, viewMenu, enabledMenu);
...
#ifdef USE_LIBID3TAG
   CMD_ADDMENUSEP("Edit ID3 Tags...", "Edit ID3 Tags", OnEditID3, projectMenu, enabledMenu);
#endif

   CMD_ADDMENUSEP("&Quick Mix", "Quick Mix", OnQuickMix, projectMenu, enabledMenu);

   CMD_ADDMENUSEP("Align Tracks Together", "Align Tracks Together", OnAlign, projectMenu, enabledMenu);
   CMD_ADDMENU("Align with Zero", "Align with Zero", OnAlignZero, projectMenu, enabledMenu);

   CMD_ADDMENUSEP("New &Audio Track", "New Audio Track", OnNewWaveTrack, projectMenu, enabledMenu);
   CMD_ADDMENU("New &Label Track", "New Label Track", OnNewLabelTrack, projectMenu, enabledMenu);

   CMD_ADDMENUSEP("&Remove Track(s)", "Remove Track(s)", OnRemoveTracks, projectMenu, enabledMenu);

   // Help menu
   CMD_ADDMENU("About Audacity...", "About Audacity", OnAbout, helpMenu, enabledMenu);

#ifndef __WXMAC__
   CMD_ADDMENUSEP("Online Help...", "Online Help", OnHelp, helpMenu, enabledMenu);
#else
   ...
   CMD_ADDMENUSEP("Run Benchmark...", "Run Benchmark", OnBenchmark, helpMenu, enabledMenu);
}
