...
"You can either drag existing files into Audacity, or use the File > Import > Audio command. \
File > Open does the same, but opens a new Audacity window for any file after the first.\
<br/><br/>\
These are the main formats Audacity will import for you: <b>AIFF, AU, FLAC, M4A</b> (only on a \
...
To change the starting point, use the |<< and |>> buttons to skip to the start or end respectively, \
...
Manual, and our Wiki [[wiki:Recording Tips|Recordin Tips]], especially the \
...
_("The main commands you will use are under the Edit menu (such as cut, copy and paste) and under the Effect \
...
<br><br>\
(2) On occasion, some of the menus or buttons are greyed out or say \"Disabled\" -  this is normal. \
For example, because you can't edit audio while playing or recording, we grey out or disable commands until you press Stop. \
Commands can sometimes be unavailable for other reasons - for example you can't run effects until you \
have audio on the screen, and you obviously can't paste audio until you've cut or copied it to Audacity's clipboard. \
<br><br>")+
ToWelcome()
         );
   }
...
When you want to listen to your work in other computer programs or burn it to CD, you \"export\" it as \
an audio file such as a WAV, AIFF or MP3. \
<br><br>\
...
screen and want to export each as a separate file. \
<br><br>\
Before you can export as <b>MP3</b>, you need to add the LAME MP3 encoder to your computer.\
...
Audacity cannot import or export files in <b>WMA, RealAudio, Shorten (SHN)</b>, or most other proprietary formats, because of licensing and patent restrictions.\
Audacity also cannot import any kind of Digital Rights Management (DRM) protected file, including most purchased online such as from iTunes or Napster.\
If you are on a Mac, you can import <b>AAC</b>  encoded files (such as <b>M4A</b>) as long as they are not DRM protected, but you cannot export to these formats.\
</p> <p>\
...
If it's a DRM-protected file, you can burn it to an audio CD in the application that is licensed to play it, then extract (rip) the CD track to WAV or AIFF.\
You can use iTunes to extract to WAV or AIFF, plus Windows Media Player 11 or <a href=\"http://cdexos.sourceforge.net/?q=download\">CDex</a> on Windows.\
Or you can play the file on your computer and record it - see <a href=\"http://audacity.sourceforge.net/help/faq?s=recording&amp;i=streaming\">.")
//#endif
         );
   }
   if(Key ==wxT("burncd") )
   {
      return WrapText(
         wxString(wxT(""))+
_("If you want to burn your work to an audio CD, you must first [[export|export as WAV or AIFF]] \
and then burn that file to CD - for more help, see [[wiki:How to burn CDs|How to burn CDs]].\
<br><br>")+
ToWelcome()
         );
   }
