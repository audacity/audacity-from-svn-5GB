...
typedef int  (*lame_encode_buffer_interleavedTYPE)(lame_global_flags *, short int[],
  int, char *, int);
        ...
        TrackList *tracks, bool selectionOnly, double t0, double t1)
{
 ...
 lame_initTYPE lame_init = (lame_initTYPE)wxDllLoader::GetSymbol(libHandle,
                                                                "lame_init");
 lame_initTYPE lame_init_params = (lame_initTYPE)wxDllLoader::GetSymbol(libHandle,
                                                                "lame_init_params");
 lame_encode_buffer_interleavedTYPE lame_encode_buffer_interleaved =
          (lame_encode_buffer_interleavedTYPE)wxDllLoader::GetSymbol(libHandle,
 ...
 if(!lame_init ||
    ...
    !lame_encode_buffer_interleaved ||
    !lame_encode_finish) {
    wxMessageBox(wxString::Format("%s is not a compatible lame encoder", libname));
    return false;
 }
 ...
 lame_init_params(&gf);
  ...
  while(t < t1 && !cancelling) {
    ...
    bytes = lame_encode_buffer_interleaved(&gf, mixed,
			  numSamples, buffer, bufferSize); // from
    ...
    if (!progress && wxGetElapsedTime(false) > 500) {
      ...
      if (selectionOnly)
        message = wxString::Format("Exporting the selected audio as an mp3");
      else
        message = wxString::Format("Exporting the entire project as an mp3");
