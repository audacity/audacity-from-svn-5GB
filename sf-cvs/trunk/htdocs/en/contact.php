<?php BoxTop($contactUsStr); ?>

<p>
If you have a question about Audacity, there are many resources:
</p>

<p>
<ul>

<li>
Read the <?php print "<a href=faq.php?$langLinkStr>$faqStr"; ?></a><br>
If you're wondering what Audacity is, or having trouble installing it
and getting it to run, please check out this page first to see if your
question is answered.
<p>

<li>
Join: <a href="http://lists.sourceforge.net/lists/listinfo/audacity-users">Audacity-Users Mailing List</a><br>
Browse: <a href="http://sourceforge.net/mailarchive/forum.php?forum_id=827">Audacity-Users Archives</a><br>
This is a new mailing list where Audacity users can talk with one another
and share their ideas.  If you think that your question could be answered
by other Audacity users, please join the mailing list and ask your
question here.  You can check the archives to see if it's already been
asked.
<p>

<li>
Email: <a href="mailto:audacity-help@lists.sourceforge.net"
>audacity-help@lists.sourceforge.net</a><br>
or Browse: <a href="http://sourceforge.net/mailarchive/forum.php?forum_id=828">Audacity-Help Archives</a><br>
<br><font size=-1><?php print "$listPrivacyStr"; ?></font>
If you're having a technical problem with Audacity, and it's too
specific to bother other Audacity users, send mail to this address.
It will be read by a team of Audacity developers and maintainers.
You should also use this address if you really need to contact the
Audacity developers directly for some other reason.
<p>
Audacity is being developed by volunteers all around the world.
Therefore, there is no phone number or mailing address where you can
reach us.  All Audacity development happens on the Internet, and 
the only way you can reach the developers is through one of these
mailing lists or email addresses.
<p>

</ul>

<?php BoxBottom(); ?>
