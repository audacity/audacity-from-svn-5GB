...
class CustomMailSendExpectation extends Swift_Plugin_MailSend
{
   ...
   */
  function CustomMailSendExpectation(&$test)
  {
    $this->Swift_Plugin_MailSend();
    $this->testcase =& $test;
  }
}
...
class TestOfMailSendPlugin extends UnitTestCase
{
   ...
   */
  function testHeadersDoNotContainThe_To_Field()
  {
   ...
   */
  function testHeadersDoNotContainThe_Subject_Field()
  {
    $swift =& new Swift(new Swift_Connection_NativeMail());
   ...
   */
  function testVitalHeadersAreIncluded()
  {
    $swift =& new Swift(new Swift_Connection_NativeMail());
   ...
   */
  function testLineEndingIsCRLFForWindows()
  {
    ...
    $body = "Foo test\r\nBar";
    $message =& new Swift_Message($subject, $body);
    $body_orig =& $message->build();
    $body_orig = $body_orig->readFull();
    $this->assertPattern("~\r\n~", $body_orig);
    $swift->send($message, $recipients, $sender);
   ...
   */
  function testLineEndingIsLFForOtherSystems()
  {
    //Windows users will be using SMTP and therefore need CRLF,
    // *nix and OS X users will be using Sendmail in "-t" mode and therefore need LF
   ...
   */
  function testRecipientsArePassedCommaSeparatedWithLWSP() //as in "no line breaks"
  {
    $swift =& new Swift(new Swift_Connection_NativeMail());
    $mailsend =& new MailSendWithGetDoMailArgs();
   ...
   */
  function testBracesAreLeftOutOfToFieldIfOSIsWindows()
  {
    $swift =& new Swift(new Swift_Connection_NativeMail());
    $mailsend =& new MailSendWithGetDoMailArgs();
    $mailsend->setOS("WIN");
   ...
   */
  function testSubjectIsProvidedInItsEncodedFormatIfNonAsciiIsUsed()
  {
    $swift =& new Swift(new Swift_Connection_NativeMail());
    $mailsend = new MailSendWithGetDoMailArgs();
    $swift->attachPlugin($mailsend, "_MAIL_SEND"); //Override the MailSend plugin with a custom mock
   ...
   */
  function testSubjectIsProvidedUnencodedIfAsciiIsUsed()
  {
    $swift =& new Swift(new Swift_Connection_NativeMail());
    $mailsend = new MailSendWithGetDoMailArgs();
    $swift->attachPlugin($mailsend, "_MAIL_SEND"); //Override the MailSend plugin with a custom mock
