 ...
 * @author Chris Corbyn <chris@w3style.co.uk>
 */
class AbstractTestOfCache extends UnitTestCase
{
  /**
  ...
  function &getCache()
  {
    $this->fail("Abstract method getCache() was not implemented.");
  }
  /**
   * Basic test to make sure data comes back from the cache in the same state it goes in.
   */
  function testDataWrittenToCacheIsRetreivable()
  {
    $cache =& $this->getCache();
    $testdata = "abc123";
   ...
   */
  function testCacheOnlyReportsToHaveCachedData()
  {
    $cache =& $this->getCache();
    $testdata = "abc123";
    $cache->write("test", $testdata);
   ...
   */
  function testCacheKeyCanBeCleared()
  {
    $cache =& $this->getCache();
    $cache->write("test", "abc");
    $this->assertTrue($cache->has("test"));
   ...
   */
  function testReturnedOutputStreamIsJustAWrapper()
  {
    $cache =& $this->getCache();
