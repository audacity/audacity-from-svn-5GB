...
//
/* EXPERIMENTAL_NOTE_TRACK CHANGE LOG:
...
void Events::expand()
{
    ...
    Allegro_event_ptr *new_events = new Allegro_event_ptr[maxlen];
    // now do copy
...
void Atoms::expand()
{
    maxlen = (maxlen + 5); // extra growth for small sizes
    maxlen += (maxlen >> 2); // add 25%
    char **new_atoms = new Attribute[maxlen];
    // now do copy
...
//Note the #ifdef, as opposed to the #ifndef that has been used above
...
void Beats::expand()
{
    maxlen = (maxlen + 5); // extra growth for small sizes
    maxlen += (maxlen >> 2); // add 25%
    Beat_ptr new_beats = new Beat[maxlen];
    // now do copy
...
{
	// the format for serialized track is given below under Alg_seq::serialize
    assert(get_type() == 't');
...
void Time_sigs::expand()
{
    maxlen = (maxlen + 5); // extra growth for small sizes
    maxlen += (maxlen >> 2); // add 25%
    Time_sig_ptr new_time_sigs = new Time_sig[maxlen];
    // now do copy
...
Alg_track_ptr Alg_seq::cut_from_track(int n, double t, 
                                      double len, bool all)
{
    return track(n)->cut(t, len, all);
}
...
Alg_track_ptr Alg_seq::copy_track(int n, double t, double len, bool all)
{
    return track(n)->copy(t, len, all);
}
...
#ifndef EXPERIMENTAL_NOTE_TRACK
void Seq::set_events(Allegro_event_ptr *events, long len, long maxlen)
{
    convert_to_seconds(); // because notes are in seconds
    notes.set_events(events, len, maxlen);
}

#else /* EXPERIMENTAL_NOTE_TRACK */
/*
void Alg_seq::set_events(Alg_event_ptr *events, long len, long maxlen)
{
    convert_to_seconds(); // because notes are in seconds
    notes.set_events(events, len, maxlen);
}
