...
* Windows: A barra de ferramentas quando mantida separada da janela principal do Audacity se mantém aberta 

mesmo que a janela principal seja minimizada. Uma forma de contornar o problema seria manter a barra 

anexada à janela.

* Linux: Placas "Full-duplex" (capazes de reproduzir e gravar ao mesmo tempo) não são suportadas neste 

ambiente. 

* Linux: Placas de som com múltiplos canais não são suportadas. 
...
Alterações na versão 1.0:
  ...
  * Mac OS:
    ...
    - O Audacity não se encerra quando a última janela de edição é fechada

    - Corrigida a falha onde a caixa de opções do Audacity não aparecia quando o ATM é carregado 

    - Corrigida a captura de áudio em amostragens baixas
      
    - Reconhecimento de arquivos MP3 por terminação ou por tipo de arquivo

    - Ganho em estabilidade e funcionalidade extendida sob a plataforma MacOS X

  * Novos recursos:

    - Opção "Salvar alterações no arquivo" em todas as plataformas suportadas

    - Descrição da ferramenta (balões no MacOS)

    - Novos atalhos de teclado

    - Faixas de rótulo agora podem ser usadas para marcar áreas de seleção

  * Unix:

    - Corrigido o problema da reprodução acelerada.

    - Corrigida a falha onde um arquivo é corrompido quando sobregravado


  * Outros:

    - Processamento de efeitos VST mais rápido

    - Correção de vários outros problemas


<br><br>

<b>Versões antigas - apenas para propósitos históricos, em inglês. </b>
